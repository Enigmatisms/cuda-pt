...
CPT_GPU_INLINE int extract_tracing_info(int obj_idx, int &hit_med_idx,
                                        bool &is_triangle) {
...
#else
constexpr CPT_GPU_INLINE bool time_in_range(const MaxDepthParams &mdp,
                                            float t) {
    return true;
}
 ...
 * x is the ptr, if x == 0, it means that active volume is 0 (not within a
 * volume)
 */
struct BankStack {
    uchar4 data;
...
// (1) Will check the obj_idx. Now, obj_idx is masked: the lower 20 bits
// represents object index, while the higher 12 bits are flag bits, if ALPHA
// flag is set true, then during this phase, the occlusion will be ignored (2)
// Will try to accumulate the transmittance along the path. This entails
// fetching the material and medium info from the GMEM so this kernel will be
// much slower (3) The occlusion_test is replaced by closest_hit shader, since
// we will need to step through the scenes
inline CPT_GPU Vec4 occlusion_transmittance_estimate(
    Ray ray, Sampler &sp, const cudaTextureObject_t bvh_leaves,
    const cudaTextureObject_t nodes, ConstF4Ptr cached_nodes,
    const PrecomputedArray &verts, const NormalArray norms,
    MediumPtrArray media, const int node_num, const int cache_num,
    BankStack nested_vols, float max_dist) {
    float total_dist = 0;
    const Vec3 inv_d = ray.d.rcp();
    Vec4 Tr(1);
    while (total_dist < max_dist) {
        int node_idx = 0, min_index = -1;
        float aabb_tmin = 0, prim_u = 0, prim_v = 0,
              min_dist = max_dist - total_dist; // FIXME: precision problem
        ...
        while (node_idx < node_num) {
            ...
            bool intersect_node =
                node.aabb.intersect(inv_d, o_div, aabb_tmin) &&
                aabb_tmin < min_dist;
            // The logic here: end_idx is reuse, if end_idx < 0, meaning that
            // the current node is non-leaf, non-leaf node stores (-all_offset)
            // as end_idx, so to skip the node and its children -end_idx will be
            // the offset. While for leaf node, 1 will be the increment offset,
            // and `POSITIVE` end_idx is stored. So the following for loop can
            // naturally run (while for non-leaf, naturally skip)
            node_idx += (!intersect_node) * (end_idx < 0 ? -end_idx : 1) +
                        int(intersect_node);
            end_idx = intersect_node ? end_idx + beg_idx : 0;
            for (int idx = beg_idx; idx < end_idx; idx++) {
                // if current ray intersects primitive at [idx], tasks will
                // store it
        ...
        if (active_medium == 0 || is_alpha_mask == false) {
            // object with no volume binding and can not be culled (alpha mode)
            // -> occlusion
            Tr.fill(0);
            break;
        }
        // if hit within range, and the normal is in the same hemisphere as that
        // of the ray direction This will mean that we are penetraing out from a
        // translucent bound
        bool is_in_medium =
            min_index >= 0 &&
            norms.eval(max(min_index, 0), prim_u, prim_v).dot(ray.d) > 0;
...
CPT_KERNEL void render_vpt_kernel(
    ...
    int num_emitter, int seed_offset, int cam_vol_idx, int node_num,
    int accum_cnt, int cache_num, int envmap_id, bool gamma_corr) {
    int px = threadIdx.x + blockIdx.x * blockDim.x,
        py = threadIdx.y + blockIdx.y * blockDim.y;
    // You see, I only provide one cam_vol_idx, meaning that camera itself can
    // not be placed in the inner levels of nested volumes. Also, this
    // cam_vol_idx is fixed, camera movement will not update it (might be
    // incorrect)
    ...
    for (int b = 0; b < md_params.max_depth; b++) {
        ...
        ray.hit_t = ray_intersect_bvh(ray, bvh_leaves, nodes, s_cached, verts,
                                      min_index, obj_info, prim_u, prim_v,
                                      node_num, cache_num, ray.hit_t);
        ...
        int material_id = 0, emitter_id = -1, hit_med_idx = 0,
            object_id =
                extract_tracing_info(obj_info, hit_med_idx, is_triangle);
        ...
        float direct_pdf = 1; // direct_pdf is the product of light_sampling_pdf
                              // and emitter_pdf
        Emitter *emitter = sample_emitter(sampler, direct_pdf, num_emitter,
                                          md.flag > 0 ? 0 : emitter_id);
        ...
        emitter_prim_id =
            emitter_prims[emitter_prim_id]; // extra mapping, introduced after
                                            // BVH primitive reordering
        Ray shadow_ray(ray.advance(md.dist), Vec3(0, 0, 0));

        // sacrificing the hemispherical sampling for envmap
        shadow_ray.d = emitter->sample(shadow_ray.o, Vec3(0, 0, 1), direct_comp,
                                       direct_pdf, sampler.next2D(), verts,
                                       norms, uvs, emitter_prim_id) -
                       shadow_ray.o;

        float emit_len_mis = shadow_ray.d.length(),
              shadow_length = emit_len_mis;
        shadow_ray.d *= __frcp_rn(emit_len_mis); // normalized direction

        // After the emitter is sampled, we first perform NEE transmittance
        // testing, for surface event we need to test whether the ray hit the
        // emitter. For medium event, emission grid should be tested but I
        // currently don't know how. After the transmittance is tested, we can

        if (emitter != c_emitter[0]) {
            nee_tr = occlusion_transmittance_estimate(
                shadow_ray, sampler, bvh_leaves, nodes, s_cached, verts, norms,
                media, node_num, cache_num, nested_vol_arr[tid],
                emit_len_mis - EPSILON);
        }
        // even if the emitter is c_emitter[0], we are still going to evaluate
        // direct component, in order to introduce fewer diverging threads.
        // Evaluating direct component is not a bottleneck
        ...
        if (md.flag > 0) { // medium event
            CONDITION_BLOCK(time_in_range(md_params, total_dist)) {
                radiance +=
                    throughput * medium->query_emission(shadow_ray.o, sampler);
            }
            ...
            ray.d =
                medium->scatter(ray.d, throughput, sampler, emission_weight);

            ray.set_delta(
                false); // currently, there is no delta lobe for phase function
        } else {
            auto it = Primitive::get_interaction(
                verts, norms, uvs, ray.advance(ray.hit_t), prim_u, prim_v,
                min_index, is_triangle);
            ...
            emission_weight =
                emission_weight /
                (emission_weight + objects[object_id].solid_angle_pdf(
            ...
            CONDITION_BLOCK(
                time_in_range(md_params, total_dist + shadow_length)) {
                radiance += throughput *
            ...
            emit_len_mis =
                direct_pdf + c_material[material_id]->pdf(it, shadow_ray.d,
                                                          ray.d, material_id) *
                                 emitter->non_delta();
            nee_thp = c_material[material_id]->eval(it, shadow_ray.d, ray.d,
                                                    material_id);

            // Bounce the ray via material scattering

            bool same_hemisphere = ray.d.dot(it.shading_norm) >
                                   0; // incident ray, whether the direction is
                                      // in the same hemisphere with normal
            ...
            // After the ray is scattered, check the scattered ray direction and
            // normal to tell whether we are exiting or entering the medium if
            // the one of the incident | exiting ray is not in the same
            // hemisphere with normal (and the other is), this will mean we are
            // not penetrating a medium interface, otherwise we should update
            // the nesting_vols
            if ((it.shading_norm.dot(ray.d) > 0 ^ same_hemisphere) == false) {
        ...
        CONDITION_BLOCK(time_in_range(md_params, total_dist + shadow_length)) {
            radiance +=
                nee_tr * throughput * direct_comp *
                (nee_thp * float(emit_len_mis > EPSILON) *
                 __frcp_rn(emit_len_mis < EPSILON ? 1.f : emit_len_mis));
        }
        total_dist += sampled_lobe != ScatterStateFlag::BSDF_NONE ? md.dist : 0;
        CONDITION_BLOCK(md_params.max_time > 0 &&
                        total_dist >= md_params.max_time) {
            break;
        }
        ...
        if (diff_b >= md_params.max_diffuse ||
            ...
            trans_b >= md_params.max_tranmit || volm_b >= md_params.max_volume)
            break;
