...
render_depth_kernel(const DeviceCamera &dev_cam, const PrecomputedArray verts,
                    const cudaTextureObject_t bvh_leaves,
                    const cudaTextureObject_t nodes, ConstF4Ptr cached_nodes,
                    DeviceImage image, float *__restrict__ output_buffer,
                    int2 *__restrict__ global_min_max, int seed_offset,
                    int node_num, int accum_cnt, int cache_num) {
    ...
    if (tid == 0)
        min_max = make_int2(ORDERED_INT_MAX, 0);
    ...
    min_dist = ray_intersect_bvh(ray, bvh_leaves, nodes, s_cached, verts,
                                 min_index, obj_info_dummy, prim_u, prim_v,
...
CPT_KERNEL static void
false_color_mapping(DeviceImage image, float *__restrict__ output_buffer,
                    int color_map_id, const int accum_cnt, const int2 min_max) {
    int px = threadIdx.x + blockIdx.x * blockDim.x,
        py = threadIdx.y + blockIdx.y * blockDim.y;
...
DepthTracer::DepthTracer(const Scene &scene)
    ...
    CUDA_CHECK_RETURN(cudaMemcpy(camera, &scene.cam, sizeof(DeviceCamera),
                                 cudaMemcpyHostToDevice));
        ...
        if (cmap_i == 0) {
            for (int i = 0; i < 256; i++) {
                color_map.emplace_back(
                    make_float4(JET_R[i], JET_G[i], JET_B[i], 1));
            }
        } else if (cmap_i == 1) {
            for (int i = 0; i < 256; i++) {
                color_map.emplace_back(
                    make_float4(PLASMA_R[i], PLASMA_G[i], PLASMA_B[i], 1));
            }
        } else {
            for (int i = 0; i < 256; i++) {
                color_map.emplace_back(
                    make_float4(VIRIDIS_R[i], VIRIDIS_G[i], VIRIDIS_B[i], 1));
            }
        }
        colormaps[cmap_i] = createArrayTexture1D<float4>(
            color_map.data(), _colormap_data[cmap_i], 256);
    }
    CUDA_CHECK_RETURN(
        cudaMemcpyToSymbol(COLOR_MAPS, &colormaps[0], sizeof(colormaps)));
}
...
CPT_CPU std::vector<uint8_t> DepthTracer::render(const MaxDepthParams &md,
                                                 int num_iter,
...
CPT_CPU void DepthTracer::render_online(const MaxDepthParams &md,
                                        bool gamma_corr) {
    min_max->x = ORDERED_INT_MAX;
    min_max->y = 0;
    ...
    accum_cnt++;
    render_depth_kernel<<<dim3(w >> SHFL_THREAD_X, h >> SHFL_THREAD_Y),
    ...
    false_color_mapping<<<dim3(w >> 5, h >> 3), dim3(32, 8)>>>(
        image, output_buffer, color_map_id, accum_cnt, *min_max);
    CUDA_CHECK_RETURN(cudaDeviceSynchronize());
...
CPT_CPU std::vector<uint8_t> DepthTracer::get_image_buffer(bool) const {
    ...
    CUDA_CHECK_RETURN(cudaMemcpy(host_floats.data(), output_buffer, copy_size,
...
CPT_CPU const float *DepthTracer::render_raw(const MaxDepthParams &md,
                                             bool gamma_corr) {
    min_max->x = ORDERED_INT_MAX;
    min_max->y = 0;
    size_t cached_size =
