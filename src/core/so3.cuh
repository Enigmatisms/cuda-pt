...
class SO3 {
  private:
    ...
    template <typename SO3Type> CPT_CPU_GPU SO3 operator+(SO3Type &&so3) const {
        return SO3(rows[0] + so3[0], rows[1] + so3[1], rows[2] + so3[2]);
    }

    template <typename SO3Type> CPT_CPU_GPU SO3 &operator+=(SO3Type &&so3) {
        rows[0] += so3[0];
        rows[1] += so3[1];
        rows[2] += so3[2];
        return *this;
    ...
    CPT_CPU_GPU SO3 scale(float val) const && {
        return SO3(rows[0] * val, rows[1] * val, rows[2] * val);
    }
  ...
  public:
    ...
    CPT_CPU_GPU const Vec3 &operator[](size_t index) const {
        return rows[index];
    }
  ...
  public:
    ...
    CPT_CPU_GPU Vec3 rotate(VecType &&v) const noexcept {
        return Vec3(rows[0].dot(v), rows[1].dot(v),
    ...
    CPT_CPU_GPU static SO3 diag(float val) {
        return SO3(Vec3(val, 0, 0), Vec3(0, val, 0), Vec3(0, 0, val));
    }

    CPT_CPU_GPU float determinant() const {
        return rows[0].x() *
                   fmaf(rows[1].y(), rows[2].z(), -rows[1].z() * rows[2].y()) -
               rows[0].y() *
                   fmaf(rows[1].x(), rows[2].z(), -rows[1].z() * rows[2].x()) +
               rows[0].z() *
                   fmaf(rows[1].x(), rows[2].y(), -rows[1].y() * rows[2].x());
    }
    ...
    CPT_CPU_GPU Vec3 inverse_transform_precomputed(Vec3 &&v, float a20,
        ...
        return Vec3(temp1.dot(v) * inv_det, temp2.dot(v) * inv_det,
                    (a20 * v.x() + a21 * v.y() + a22 * v.z()) * inv_det);
    }
...
CPT_CPU_GPU_INLINE SO3 skew_symmetry(const Vec3 &v) {
    return SO3(Vec3(0, -v.z(), v.y()), Vec3(v.z(), 0, -v.x()),
               Vec3(-v.y(), v.x(), 0));
}

CPT_CPU_GPU_INLINE SO3 vec_mul(const Vec3 &v1, const Vec3 &v2) {
    return SO3(v2 * v1.x(), v2 * v1.y(), v2 * v1.z());
}

// This can be improved (maybe not, Rodrigues transformation is already good
// enough)
...
// other wise, `to` is actually the `from` vector (from world to local, local is
// (0, 0, 1))
template <typename VecType>
CPT_GPU SO3 rotation_fixed_anchor(VecType &&to, bool l2w) {
...
CONDITION_TEMPLATE(VecType, Vec3)
CPT_GPU_INLINE Vec3 delocalize_rotate(VecType &&anchor, const Vec3 &to,
                                      const Vec3 &input) {
