...
__global__ void create_bsdf(BSDF** dst, Vec4 k_d, Vec4 k_s, Vec4 k_g, int kd_tex_id = 0, int ex_tex_id = 0) {
...
__global__ void create_point_source(Emitter* &dst, Vec4 le, Vec3 pos) {
    if (threadIdx.x == 0 && blockIdx.x == 0) {
        dst = new PointSource(std::move(le), std::move(pos));
    }
}

__global__ void create_area_source(Emitter* &dst, Vec4 le, int obj_ref, bool is_sphere) {
    if (threadIdx.x == 0 && blockIdx.x == 0) {
