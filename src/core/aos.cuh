...
template <typename StructType> class SoA3 {
  public:
    StructType *data;
    size_t size;
  ...
  public:
    CPT_CPU SoA3(size_t _size) : size(_size) {
        CUDA_CHECK_RETURN(
            cudaMallocManaged(&data, 3 * sizeof(StructType) * _size));
    }
    ...
    CPT_GPU SoA3(StructType *_data, size_t size) : data(_data), size(size) {}

    CPT_CPU void
    from_vectors(const std::vector<StructType> &vec1,
    ...
    CPT_CPU_GPU_INLINE const StructType &x(int index) const {
        return data[index];
    }
    CPT_CPU_GPU_INLINE const StructType &y(int index) const {
        return data[index + size];
    }
    CPT_CPU_GPU_INLINE const StructType &z(int index) const {
    ...
    CPT_CPU_GPU_INLINE StructType &z(int index) {
        return data[index + (size << 1)];
    }

    CPT_GPU_INLINE StructType eval(int index, float u, float v) const {
        return StructType{};
    }
};
...
CPT_GPU_INLINE Vec3 SoA3<Vec3>::eval(int index, float u, float v) const {
    return (data[index] * (1.f - u - v) + data[index + size] * u +
            data[index + (size << 1)] * v)
        .normalized();
};
...
// This class can be both SoA and AoS, depending on the actual compilation
// setting
class PrecomputedArray {
  public:
    Vec4 *data; // packed sphere data or vertex difference (with adjoint matrix
                // terms packed)
    size_t size;
  ...
  public:
    ...
    // vertices input, convert to float4 and store precomputed adjoint matrix
    // value convert from vertices to vertex difference
    CPT_CPU void from_vectors(const std::vector<Vec3> &vec1,
                              const std::vector<Vec3> &vec2,
                              const std::vector<Vec3> &vec3,
                              const std::vector<bool> *const sphere_flags =
                                  nullptr // use naked ptr to enable null
    ) {
        for (size_t i = 0; i < size; i++) {
            if (sphere_flags &&
                sphere_flags->at(
                    i)) { // sphere is compacted into 4 floats, fast loading!
                Vec3 center = vec1[i];
                data[INDEX_X(i, size)] =
                    Vec4(center.x(), center.y(), center.z(), vec2[i].x());
                data[INDEX_Y(i, size)] = Vec4();
                data[INDEX_Z(i, size)] = Vec4();
                continue;
            }
            Vec3 abs1 = vec1[i], diff1 = vec2[i] - abs1, diff2 = vec3[i] - abs1;
            // precomputed adjacent matrix terms, work as padding (saved 3
            // FMADD, 3 FMUL and 3 FADD per triangle)
    ...
    CPT_CPU_GPU_INLINE Vec4 x(int index) const {
        return data[INDEX_X(index, size)];
    }
    CPT_CPU_GPU_INLINE Vec4 y(int index) const {
        return data[INDEX_Y(index, size)];
    }
    CPT_CPU_GPU_INLINE Vec4 z(int index) const {
        return data[INDEX_Z(index, size)];
    }

    CPT_CPU_GPU_INLINE float2 x_front(int index_2) const {
        return (
            reinterpret_cast<const float2 *>(data))[(index_2 << 1) + index_2];
    }
    CPT_CPU_GPU_INLINE float2 y_front(int index_2) const {
        return (reinterpret_cast<const float2 *>(
            data))[(index_2 << 1) + index_2 + 2];
    }
    CPT_CPU_GPU_INLINE float2 z_front(int index_2) const {
        return (reinterpret_cast<const float2 *>(
            data))[(index_2 << 1) + index_2 + 4];
    }

    CPT_CPU_GPU_INLINE float2 x_back(int index_2) const {
        return (reinterpret_cast<const float2 *>(
            data))[(index_2 << 1) + index_2 + 1];
    }
    CPT_CPU_GPU_INLINE float2 y_back(int index_2) const {
        return (reinterpret_cast<const float2 *>(
            data))[(index_2 << 1) + index_2 + 3];
    }
    CPT_CPU_GPU_INLINE float2 z_back(int index_2) const {
        return (reinterpret_cast<const float2 *>(
            data))[(index_2 << 1) + index_2 + 5];
    }
    ...
    CPT_CPU_GPU_INLINE Vec3 get_sphere_point(const Vec3 &normal,
                                             int index) const {
        auto v = data[INDEX_Z(index, size)];
        return normal * v.w() +
               Vec3(v.x(), v.y(), v.z()); // center + radius * direction
    }
};
...
class NormalAoSArray {
  private:
    ...
    float4 *data; // packed sphere data or vertex difference (with adjoint
                  // matrix terms packed)
    size_t pitch;
  ...
  public:
    ...
    CPT_CPU void from_vectors(const std::vector<Vec3> &vec1,
        ...
        CUDA_CHECK_RETURN(
            cudaMallocPitch(&data, &pitch, width * sizeof(float4), height));
        CUDA_CHECK_RETURN(cudaMemcpy2D(data, pitch, temp_data, host_pitch,
                                       host_pitch, height,
    ...
    CPT_GPU_INLINE Vec3 eval(int index, float u, float v) const {
        ...
        index %= 2048;         // width
        // Vec4 lerp_v =
        //     Vec4(tex2D<float4>(tex_obj, float(TRI_IDX(index) + 0) + (1.f - u
        //     - v) / (1.f - v), y)) * (1.f - v) + Vec4(tex2D<float4>(tex_obj,
        //     float(TRI_IDX(index) + 2), y)) * v;
        ...
        return (Vec3(n1.xyz()) * (1.f - u - v) + Vec3(n2.xyz()) * u +
                Vec3(n3.xyz()) * v)
            .normalized();
    }
};
...
template <typename Ty> class ConstBuffer {
  private:
    Ty *_buffer;
    size_t _size;
  ...
  public:
    CPT_CPU ConstBuffer(size_t _sz) : _size(_sz) {
        if (_size <= 0) {
            throw std::runtime_error(
                "Allocated buffer size is not a positive value.\n");
        }
    ...
    CPT_CPU ConstBuffer(const Ty *const inputs, size_t _sz) : _size(_sz) {
        ...
        CUDA_CHECK_RETURN(cudaMallocManaged(&_buffer, sizeof(Ty) * _sz));
        CUDA_CHECK_RETURN(cudaMemcpy(_buffer, inputs, sizeof(Ty) * _sz,
                                     cudaMemcpyHostToDevice));
    }
    ...
    CPT_CPU_GPU_INLINE const Ty &operator[](int index) const {
...
template <typename InnerType> using ArrayType = SoA3<InnerType>;
