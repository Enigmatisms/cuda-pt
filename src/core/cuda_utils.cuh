...
#define UINT_CREF_CAST(v) reinterpret_cast<const uint32_t &>(v)

#define CONDITION_TEMPLATE(VecType, TargetType)                                \
    template <typename VecType, typename = std::enable_if_t<std::is_same_v<    \
                                    std::decay_t<VecType>, TargetType>>>

#define CONDITION_TEMPLATE_DEFAULT(VecType, TargetType)                        \
    template <typename VecType,                                                \
              std::enable_if_t<                                                \
                  std::is_same_v<std::decay_t<VecType>, TargetType>, int> = 0>
...
#define CONDITION_TEMPLATE_SEP_3(T1, T2, T3, TargetType1, TargetType2,         \
                                 TargetType3)                                  \
...
template <typename T> CPT_CPU_GPU_INLINE T sgn(T val) {
...
CPT_CPU_INLINE int to_int_linear(float x) {
    return int(std::clamp(x, 0.f, 1.f) * 255.f);
}
CPT_CPU_INLINE int to_int(float x) {
    return int(powf(std::clamp(x, 0.f, 1.f), 1.f / 2.1f) * 255.f + .5f);
}
