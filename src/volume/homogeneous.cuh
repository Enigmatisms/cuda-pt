...
// FIXME：no need to inline the functions since virtual functions won't be
// inlined (base class ptr)
class HomogeneousMedium : public Medium {
  ...
  public:
    ...
    CPT_CPU_GPU HomogeneousMedium(VType1 &&_sigma_a, VType2 &&_sigma_s)
        : sigma_a(std::forward<VType1>(_sigma_a)),
          sigma_s(std::forward<VType2>(_sigma_s)) {
    ...
    CPT_GPU_INLINE MediumSample sample(
        ...
        float exp_y_x = expf(-(sigma_t.y() - sigma_t.x()) * msp.dist),
              ...
              exp_x_y = __frcp_rn(exp_y_x), exp_x_z = __frcp_rn(exp_z_x),
              exp_y_z = __frcp_rn(exp_z_y);
        /**
         * Why so complicated?
         *
         * If you implement in the similar way as PBRT-v3
         * You will find that when PDF is pretty small, you will need to clip it
         * to prevent numerical instability, but this can be detrimental to
         * rendering results: unexpected stripes will appear in the media. If
         * you choose not to clip the results, you might wind up having black
         * dots.
         *
         ...
         */
        msp.local_thp =
            is_medium ? Vec4(3.f / (sigma_t.x() + sigma_t.y() * exp_y_x +
                                    sigma_t.z() * exp_z_x),
                             3.f / (sigma_t.x() * exp_x_y + sigma_t.y() +
                                    sigma_t.z() * exp_z_y),
                             3.f / (sigma_t.x() * exp_x_z +
                                    sigma_t.y() * exp_y_z + sigma_t.z())) *
                            sigma_s
                      : Vec4(3.f / (1.f + exp_y_x + exp_z_x),
