...
CPT_KERNEL void compute_volume_sum(const nanovdb::FloatGrid *density,
                                   float *sum_avg, int *valid_cell,
                                   int num_cells, int width, int height,
...
CPT_GPU GridVolumeMedium::GridVolumeMedium(const nanovdb::FloatGrid *_den,
                                           float _avg_density,
                                           const nanovdb::Vec3fGrid *_alb,
                                           const nanovdb::FloatGrid *_em,
                                           Vec3 _const_alb, float _scale,
                                           float _temp_scale, float _em_scale)
    : density(_den), albedo(_alb), emission(_em),
      const_alb(std::move(_const_alb)), scale(_scale), temp_scale(_temp_scale),
      emission_scale(_em_scale), avg_density(_avg_density) {
    auto bbox = density->worldBBox();
    grid_aabb = AABB(Vec3(bbox.min()[0], bbox.min()[1], bbox.min()[2]),
                     Vec3(bbox.max()[0], bbox.max()[1], bbox.max()[2]), 0, 0);
}
...
CPT_GPU Vec4 GridVolumeMedium::query_emission(Vec3 pos, Sampler &sp) const {
    ...
    auto res =
        Vec4(tex1D<float4>(emit_tex, temp * temp_scale)) * emission_scale;
    return res;
}

CPT_GPU_INLINE MediumSample GridVolumeMedium::sample(const Ray &ray,
                                                     Sampler &sp,
...
CPT_GPU_INLINE Vec4 GridVolumeMedium::transmittance(const Ray &ray, Sampler &sp,
                                                    float dist) const {
...
CPT_GPU MediumSample GridVolumeMedium::delta_tracking_dist_sample(
    const Ray &ray, Sampler &sp, float near_t, float far_t) const {
    float t, inv_maj;
    ...
    while (t < far_t) {
        ...
        if (sp.next1D() < d * inv_maj) { // real collision
            ds.dist = t;
            ds.local_thp =
                sample_albedo(std::move(sample_pos), std::move(offset));
            ds.flag = 1;
            break;
        }
...
CPT_KERNEL void
create_grid_volume(const nanovdb::FloatGrid *dev_den_grids,
                   const nanovdb::Vec3fGrid *dev_alb_grids,
                   const nanovdb::FloatGrid *dev_ems_grids, Medium **media,
                   PhaseFunction **phases, int med_id, int ph_id, Vec3 _calb,
                   float scale, float temp_scale, float em_scale,
                   float avg_density // for residual ratio tracking
) {
    if (threadIdx.x == 0) {
        media[med_id] = new GridVolumeMedium(
            dev_den_grids, avg_density, dev_alb_grids, dev_ems_grids,
            std::move(_calb), scale, temp_scale, em_scale);
...
CPT_CPU void GridVolumeManager::push(size_t med_id, size_t ph_id,
                                     std::string den_path, float scale,
                                     float temp_scale, float em_scale,
                                     std::string alb_path,
                                     std::string ems_path) {
...
CPT_CPU void GridVolumeManager::push(size_t med_id, size_t ph_id,
                                     std::string den_path, Vec4 albedo,
                                     float scale, float temp_scale,
                                     float em_scale, std::string ems_path) {
    medium_indices.push_back(med_id);
    phase_indices.push_back(ph_id);
    scales.push_back(scale);
    temp_scales.push_back(temp_scale);
...
CPT_CPU void GridVolumeManager::to_gpu(Medium **medium,
    ...
    for (int i = 0; i < medium_indices.size(); i++) {
        size_t media_idx = medium_indices[i], phase_idx = phase_indices[i];
        ...
        int num_cells =
            (bbox_diff.x() + 1) * (bbox_diff.y() + 1) * (bbox_diff.z() + 1);
        ...
        compute_volume_sum<<<((num_cells + 1023) >> 10), 1024>>>(
            den_grids[i], sum_val, cnt_val, num_cells, bbox_diff.x() + 1,
            bbox_diff.y() + 1, bbox_min.x(), bbox_min.y(), bbox_min.z());
        CUDA_CHECK_RETURN(cudaMemcpy(&host_val, sum_val, sizeof(float),
                                     cudaMemcpyDeviceToHost));
        CUDA_CHECK_RETURN(cudaMemcpy(&host_cnt, cnt_val, sizeof(int),
                                     cudaMemcpyDeviceToHost));

        CUDA_CHECK_RETURN(cudaFree(sum_val));
        CUDA_CHECK_RETURN(cudaFree(cnt_val));

        create_grid_volume<<<1, 1>>>(
            den_grids[i], alb_grids[i], ems_grids[i], medium, phases, media_idx,
            phase_idx, const_albedos[i], scales[i], temp_scales[i],
            em_scales[i], host_val / float(host_cnt));
    }
...
CPT_CPU void GridVolumeManager::load_black_body_data(std::string path_prefix) {
    ...
    if (!file) {
        std::cerr
            << "Failed to open binary file containing blackbody emission data."
            << std::endl;
    ...
    _emit_tex =
        createArrayTexture1D<float4>(data.data(), _bb_emission, data.size());
    CUDA_CHECK_RETURN(
        cudaMemcpyToSymbol(emit_tex, &_emit_tex, sizeof(cudaTextureObject_t)));
}
