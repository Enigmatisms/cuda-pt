...
static cudaTextureObject_t create_texture2d(const TexType *host_data, int width,
    ...
    size_t pitch;
    CUDA_CHECK_RETURN(
        cudaMallocPitch(&d_ptr, &pitch, width * sizeof(TexType), height));

    size_t host_pitch = width * sizeof(TexType);
    CUDA_CHECK_RETURN(cudaMemcpy2D(d_ptr, pitch, host_data, host_pitch,
                                   host_pitch, height, cudaMemcpyHostToDevice));
