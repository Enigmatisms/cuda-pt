...
def update_density_value_in_xml(
    xml_path: str, new_density_path: str, new_emission_path: str = ""
):
    """
    ...
    for medium in root.findall(
        ".//medium[@type='grid']"
    ):  # find all <medium type="grid">
...
def parse_args():
    ...
    parser.add_argument(
        "--volume_path", type=str, required=True, help="Path to the VDB volume files."
    )
    parser.add_argument(
        "--replace_path_dn",
        type=str,
        required=True,
        help="Path to substitute (density) for in the xml file.",
    )
    parser.add_argument(
        "--output_path",
        type=str,
        required=True,
        help="Path to the output rendering file.",
    )
    parser.add_argument(
        "--output_name", type=str, required=True, help="Name of the output image."
    )
    parser.add_argument(
        "--emission_path",
        type=str,
        default="",
        help="Path to the VDB volume emission files.",
    )
    parser.add_argument(
        "--replace_path_em",
        type=str,
        default="",
        help="Path to substitute (emission) for in the xml file.",
    )
...
def ddp_main(local_rank, args, file_index):
    ...
    if local_rank == args.device_id_offset:
        ...
        output_file = os.path.join(
            args.output_path, f"{args.output_name}_{file_index:03d}.png"
        )
...
def job_steady_vdb_serial(args):
    ...
    for i, (file_dn, file_em) in enumerate(zip(files_dn, files_em)):
        ...
        CONSOLE.rule()
        CONSOLE.log(
            f"({i + 1:3d} / {len(files_dn):3d}) Current file: '{file_dn}' | '{file_em}'"
        )
        CONSOLE.rule()
