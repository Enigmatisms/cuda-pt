...
def signal_handler(signum, frame):
    global EXIT_FLAG
    CONSOLE.log(
        f"[Rank={dist.get_rank()}] Received signal {signum}. Setting EXIT_FLAG to True."
    )
    EXIT_FLAG = True
...
def reduce_rendered_image(
    img: torch.Tensor, this_spp: int, device, spp_total=None
) -> torch.Tensor:
...
def parse_args():
    ...
    parser.add_argument(
        "--reduce_interval", type=int, default=128, help="Interval for reduce/average"
    )
    parser.add_argument(
        "--ft_interval",
        type=int,
        default=64,
        help="Interval for recording the frame time",
    )
    parser.add_argument(
        "--max_iterations",
        type=int,
        default=100000,
        help="Number of total iterations (render steps)",
    )
    parser.add_argument(
        "--device_id_offset", type=int, default=0, help="If an offset is needed."
    )

    parser.add_argument(
        "--scene_path", type=str, required=True, help="Path to the scene XML file."
    )

    parser.add_argument(
        "--rm_logs", default=False, action="store_true", help="Remove previous logs"
    )
    parser.add_argument(
        "--record_var",
        default=False,
        action="store_true",
        help="Record variance curves",
    )
...
def ddp_main(local_rank, args):
    ...
    if not os.path.exists(args.scene_path):
        CONSOLE.log(
            f"[yellow][WARN] Scene path '{args.scene_path}' does not exist. Exiting... [/yellow]"
        )
        exit(0)
    ...
    renderer = PythonRenderer(
        args.scene_path, local_rank, local_rank * OFFSET_SCALER + random.randint(0, 1e8)
    )
    ...
    pbar = tqdm.tqdm(
        range(args.max_iterations), disable=(local_rank != args.device_id_offset)
    )
    for step in pbar:
        if EXIT_FLAG:
            if is_main_proc:
        ...
        if (step + 1) % args.reduce_interval == 0:
            image_avg, spp_total = reduce_rendered_image(
                image, renderer.counter(), device
            )

            if local_rank == args.device_id_offset:
                image_avg = image_avg.clamp(0, 1)
                image_avg_cpu = (
                    image_avg[..., :-1].detach().cpu().permute(2, 0, 1)
                )  # (3, H, W)
            ...
            if args.record_var:
                ...
                if var_image is not None and var_image.numel() is not None:
                    var_avg, _ = reduce_rendered_image(
                        var_image, renderer.counter(), device, spp_total
                    )
                    if local_rank == args.device_id_offset:
                        writer.add_scalar(
                            "variance/Average Variance",
                            var_avg.mean() * world_size / spp_total,
                            step + 1,
                        )
        ...
        if (step + 1) % args.ft_interval == 0:
            ...
            if local_rank == args.device_id_offset:
                writer.add_scalars(
                    ...
                    {
                        f"Frame Time/rank-{i}": ft_val
                        for i, ft_val in enumerate(gather_list)
                    },
                    global_step=step + 1,
                )
